...
function Todo({ id, todo, completed }) {
  ...
  const toggleComplete = (e) => {
    console.log("Valie-->", e.currentTarget.checked);
    updateTodo(id, {
        id: id,
        todo: todoMsg,
        completed: !completed
  ...
  const toggleEditable = () => {
    ...
    if (isEditable) {
        console.log("value updated-->", isEditable);
        updateTodo(id, {
            id: id,
            todo: todoMsg,
            completed: completed
  ...
  return (
    <div className={`flex border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300  text-black ${isCompleted ? "bg-[#c6e9a7]" : "bg-[#ccbed7]"}`}>
      ...
      />
      <input
        className={`border outline-none w-full bg-transparent rounded-lg ${isCompleted ? 'line-through' : 'no-underline'} ${isEditable ? "border-black/10 px-2" : "border-transparent"}`}
        type="text"
        value={todoMsg}
      ...
      disabled={isCompleted} onClick={() => {deleteTodo(id); }}>
        <svg
